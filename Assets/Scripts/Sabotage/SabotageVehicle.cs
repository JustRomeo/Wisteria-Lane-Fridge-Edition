...
public class SabotageVehicle : MonoBehaviour
{
    private int budget;
    ...
    public GameObject deliveryLine;
    ...
    void SabotageRequestRPC(int successPercent, int type, int playerid) {
        if (playerid != PlayerManager.playerID) {
            ...
            if (randomSuccess <= successPercent) {
                bool carDamage = deliveryLine.GetComponent<DeliveryChain>().isVehicleDamaged();
                if (carDamage == false)
                    deliveryLine.GetComponent<DeliveryChain>().vehicleSabotage(type);
